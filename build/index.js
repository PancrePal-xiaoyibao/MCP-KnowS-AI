...
async function main() {
    ...
    server.tool("knows_ai_search", "Search for clinical evidence and get a question_id. Always the first step before answering.", {
    ...
    server.tool("knows_answer", "Generate a single scenario-based answer using a question_id from search results.", {
    ...
    server.tool("knows_batch_answer", "Batch generate answers for multiple questions concurrently. Preferred over knows_answer for multiple items.", {
    ...
    server.tool("knows_evidence_summary", "Get an AI-generated summary for a specific evidence ID.", {
    ...
    server.tool("knows_evidence_highlight", "Retrieve highlighted text segments from the original evidence source.", {
        evidence_id: z.string().min(1),
    }, async (args) => {
    ...
    server.tool("knows_get_paper_en", "Get detailed content of an English paper.", {
        evidence_id: z.string().min(1),
    ...
    server.tool("knows_get_paper_cn", "Get detailed content of a Chinese paper.", {
        evidence_id: z.string().min(1),
    }, async (args) => {
    ...
    server.tool("knows_get_guide", "Get detailed content of a clinical guideline.", {
        evidence_id: z.string().min(1),
        translate_to_chinese: z.boolean().optional(),
    }, async (args) => {
    ...
    server.tool("knows_get_meeting", "Get detailed content of a medical meeting abstract.", {
        evidence_id: z.string().min(1),
        translate_to_chinese: z.boolean().optional(),
    }, async (args) => {
    ...
    server.tool("knows_auto_tagging", "Automatically extract tags and structured elements from content.", {
    ...
    server.tool("knows_list_question", "List historical questions.", {
    ...
    server.tool("knows_list_interpretation", "List historical interpretations.", {
        from_time: z.number().optional(),
        to_time: z.number().optional(),
        page: z.number().optional(),
        page_size: z.number().optional(),
    ...
    server.tool("knows_batch_get_evidence_details", "Batch retrieve details for multiple evidences (Paper/Guide/Meeting). Highly recommended for retrieving > 1 items.", {
    ...
    server.resource("evidence", "knows://evidence/{type}/{id}", async (uri) => {
