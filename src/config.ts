export type KnowsConfig = {
  apiKey: string;
  baseUrl: string;
  logLevel: string;
  defaultDataScope: string[]; // 默认证据搜索范围
};

export function loadConfig(): KnowsConfig {
  ...
  let defaultDataScope: string[] = ["PAPER", "PAPER_CN", "GUIDE", "MEETING"]; // 兜底默认值：全开
